...
export default function ShopOrders({match}) {
  ...
  //we will fetch and list the order for the given shop, and for each order,
  //we'll render the order details along with the list of products that were ordered in React component named ProductOrderEdit


  //we will retrieve the list of orders for the provided shop ID from the db and set it to the state to be rendered in the view
  //we make a request to the backend API to list orders by shop using the listByShop fetch method
  //set the retrieved orders to the state in a useEffect hook
...
//we will iterate through the list of orders and render each order in a collapsible list from Material-UI, which expand when clicked

   //this component will display the purchased products and allow seller to edit status of each product
                //the updateOrders method is passed as a prop to this component so that the status can be updated when a product status is changed
